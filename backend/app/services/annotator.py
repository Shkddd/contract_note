...
def run_review(doc_id: int) -> dict:
    """Run a full review: compare all clauses against KB, generate annotations."""
    db = get_db()
    ...
    # Clear previous annotations and review for this doc
    clause_ids = [c["id"] for c in clauses]
    for cid in clause_ids:
        db.execute("DELETE FROM annotations WHERE clause_id = ?", (cid,))
    ...
    for clause in clauses:
        ...
        kb_matches = semantic_search(clause_text, top_k=3)

        if not kb_matches:
            # No KB match — mark as unchecked (not necessarily missing)
            continue

        # Use LLM to analyze this clause against matched KB entries
        if settings.llm_api_key and kb_matches:
            analysis = analyze_clause_with_llm(clause_text, kb_matches, settings)
        else:
            # Simple heuristic match
            analysis = {
                "match_type": "match",
                "risk_level": kb_matches[0].get("risk_level", "中"),
                "comment": f"与知识库「{kb_matches[0]['title']}」相关",
                "suggestion": "请人工审核",
            }
        ...
        db.execute(
            ...
            (
                clause["id"],
                kb_matches[0]["id"] if kb_matches else None,
                kb_matches[0]["title"] if kb_matches else "",
                analysis["match_type"],
                analysis["risk_level"],
                analysis["comment"][:500],
                analysis["suggestion"][:500],
            ),
        )
